...
impl ManagedStmt {
  ...
  pub fn column_name(&self, i: i32) -> Result<&str, ResultCode> {
    ...
    if ptr.is_null() {
      ...
      Ok(unsafe { core::str::from_utf8_unchecked(core::ffi::CStr::from_ptr(ptr).to_bytes()) })
    }
  }
  ...
  pub fn into_raw(self) -> *mut stmt { // should be ok as statement is created using sqlite api
    let stmt = self.stmt;
...
impl Context for *mut context {
  ...
  /// Copies the blob into SQLite's memory and will drop at end of scope rust side.
  #[inline]
  fn result_blob_owned(&self, blob: Vec<u8>) {
    let len = blob.len();
    let ptr = unsafe { sqlite_alloc(len) };
  ...
  fn set(&self, val: &str) {
    unsafe {
      ...
      if let Ok(s) = CString::new(val) {
        **self = copy_into_sqlite_mem_cstring(s);
      } else {
...
impl VTabRef for *mut *mut vtab {
  fn set<T>(&self, val: Box<T>) {
    unsafe {
      let raw_val = Box::into_raw(val); // they seem dropped when disconnect is called
...
impl CursorRef for *mut *mut vtab_cursor {
  fn set<T>(&self, val: Box<T>) {
    unsafe {
      let raw_val = Box::into_raw(val); // they seem dropped when cursor is closed
  ...
  fn set_err(&self, val: &str) {
    unsafe {
      ...
      if let Ok(e) = CString::new(val) {
        (**self).zErrMsg = copy_into_sqlite_mem_cstring(e);
      }
    }
  }
}
...
pub fn copy_into_sqlite_mem_cstring(s: CString) -> *mut c_char {
...
pub fn into_sqlite_mem_cstring(s: &str) -> *mut c_char {
  let len = s.as_bytes().len();
  // add null terminator
  let ptr = unsafe { sqlite_alloc(len + 1) };
  unsafe {
